...
void PrintUsage()
{
	...
	cout << "\t-r = Used to retrieve an image from FLASH and save it to disk." << endl << endl;
...
int main(int argc, char** argv)
{
	...
	{
		cout << "This camera does not support the data flash feature... press Enter to exit." << endl;
		cin.ignore();
		return -1;
	}
	...
	if ( image.GetDataSize() > dataFlashSize )
	{
		...
		cout << "Using Format7 image of size " << imageWidth << "x" << imageHeight << ", " << bytesPerPixel << " bytes per pixel." << endl;
	...
	else if ( flashMode == RETRIEVE )
	{
		...
		cout << "Successfully retrieved image from camera to " << filename << endl;
	}
